...
exports.initDatabase = exports.get = exports.run = exports.query = void 0;
...
const dbPath = path_1.default.isAbsolute(dbFile) ? dbFile : path_1.default.join(process.cwd(), dbFile);
...
const initDatabase = async () => {
  ...
  `);
    try {
        const columns = await (0, exports.query)("PRAGMA table_info(complaints)");
        const columnNames = columns.map(c => c.name);
        if (!columnNames.includes('state')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN state TEXT");
        }
        if (!columnNames.includes('district')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN district TEXT");
        }
        if (!columnNames.includes('taluk')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN taluk TEXT");
        }
        if (!columnNames.includes('revenue_division')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN revenue_division TEXT");
        }
        if (!columnNames.includes('firka')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN firka TEXT");
        }
        if (!columnNames.includes('village_panchayat')) {
            await (0, exports.run)("ALTER TABLE complaints ADD COLUMN village_panchayat TEXT");
        }
  ...
  `);
    ...
    const categoryCount = await (0, exports.get)('SELECT COUNT(*) as count FROM categories');
    if (categoryCount.count === 0) {
        ...
        for (const cat of defaultCategories) {
            await (0, exports.run)('INSERT INTO categories (name) VALUES (?)', [cat]);
        }
        console.log('Categories seeded.');
    }
    // Seed Departments if empty
    const deptCount = await (0, exports.get)('SELECT COUNT(*) as count FROM departments');
    if (deptCount.count === 0) {
        ...
        for (const dept of defaultDepts) {
            await (0, exports.run)('INSERT INTO departments (name) VALUES (?)', [dept]);
        }
        console.log('Departments seeded.');
    }
    // Seed Users if empty
    const userCount = await (0, exports.get)('SELECT COUNT(*) as count FROM users');
    if (userCount.count === 0) {
        const salt = await bcryptjs_1.default.genSalt(10);
        const adminPassword = await bcryptjs_1.default.hash('admin123', salt);
        const citizenPassword = await bcryptjs_1.default.hash('citizen123', salt);
        await (0, exports.run)(`
