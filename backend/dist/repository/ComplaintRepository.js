...
class ComplaintRepository {
    async create(complaint) {
        const result = await (0, db_js_1.run)(`INSERT INTO complaints (
    ...
    async findById(id) {
        ...
        const row = await (0, db_js_1.get)(`SELECT c.*, cat.name as category_name, dept.name as department_name, u.name as citizen_name, u.email as citizen_email
       FROM complaints c
    ...
    async update(id, complaint) {
        const fields = [];
        const values = [];
        if (complaint.title !== undefined) {
            fields.push('title = ?');
            values.push(complaint.title);
        }
        if (complaint.description !== undefined) {
            fields.push('description = ?');
            values.push(complaint.description);
        }
        if (complaint.category_id !== undefined) {
            fields.push('category_id = ?');
            values.push(complaint.category_id);
        }
        if (complaint.department_id !== undefined) {
            fields.push('department_id = ?');
            values.push(complaint.department_id);
        }
        if (complaint.priority !== undefined) {
            fields.push('priority = ?');
            values.push(complaint.priority);
        }
        if (complaint.status !== undefined) {
            fields.push('status = ?');
            values.push(complaint.status);
        }
        if (complaint.address !== undefined) {
            fields.push('address = ?');
            values.push(complaint.address);
        }
        if (complaint.landmark !== undefined) {
            fields.push('landmark = ?');
            values.push(complaint.landmark);
        }
        if (complaint.latitude !== undefined) {
            fields.push('latitude = ?');
            values.push(complaint.latitude);
        }
        if (complaint.longitude !== undefined) {
            fields.push('longitude = ?');
            values.push(complaint.longitude);
        }
        if (complaint.contact_number !== undefined) {
            fields.push('contact_number = ?');
            values.push(complaint.contact_number);
        }
        fields.push('updated_at = CURRENT_TIMESTAMP');
        if (fields.length === 1)
            return false; // Only updated_at
        values.push(id);
        const result = await (0, db_js_1.run)(`UPDATE complaints SET ${fields.join(', ')} WHERE id = ?`, values);
        return result.changes > 0;
    }
    async delete(id) {
        const result = await (0, db_js_1.run)('DELETE FROM complaints WHERE id = ?', [id]);
        return result.changes > 0;
    }
    // Attachments
    async createAttachment(complaintId, fileUrl) {
        const result = await (0, db_js_1.run)('INSERT INTO attachments (complaint_id, file_url) VALUES (?, ?)', [complaintId, fileUrl]);
        return { id: result.lastID, complaint_id: complaintId, file_url: fileUrl };
    }
    async getAttachments(complaintId) {
        const rows = await (0, db_js_1.query)('SELECT * FROM attachments WHERE complaint_id = ? ORDER BY id DESC', [complaintId]);
        return rows;
    }
    // Filtered queries
    async queryComplaints(filters, citizenId) {
        let whereClauses = [];
        let params = [];
        ...
        // Count Query
        const totalRow = await (0, db_js_1.get)(`SELECT COUNT(*) as count 
       FROM complaints c 
       ${whereSql}`, params);
      ...
      ${limitSql}
    `;
        const data = await (0, db_js_1.query)(dataSql, params);
    ...
    async getComplaintsCountByStatus(citizenId) {
    ...
    async getRecentTimeline(limit = 8) {
        return await (0, db_js_1.query)(`
    ...
    async getResolvedComplaintsMonthly() {
        return await (0, db_js_1.query)(`
      SELECT strftime('%Y-%m', created_at) as month, COUNT(*) as count 
      FROM complaints 
    ...
    async getCategoryShareDistribution() {
        return await (0, db_js_1.query)(`
